...
from .scraperresult import TwitterScraperResultProfile, TwitterScraperTrends, TwitterSearchKeywords, TwitterScraperTweets,TwitterScraperFollowers,TwitterScraperFollowing,TwitterScraperFavorites
...
class TwitterScraper:
	...
	def get_followers(self,user: str=None,count: int=20) :
		i = 0
		tweets = []

		resp = self.__requestsdata(
			url=self.url,
			target=f"i/api/1.1/followers/ids.json?cursor=-1&screen_name={user}&count={count}"
		)
		...
		return TwitterScraperFollowers(
			twitter_data=data
		)

	def get_favorites(self,user: str=None,count: int=20) :
		i = 0
		tweets = []

		resp = self.__requestsdata(
			url=self.url,
			target=f"i/api/1.1/favorites/list.json?cursor=-1&screen_name={user}&count={count}"
		)
		...
		return TwitterScraperFavorites(
			twitter_data=data
		)
	def get_following(self,user: str=None,count: int=20) :
		i = 0
		tweets = []

		resp = self.__requestsdata(
			url=self.url,
			target=f"i/api/1.1/friends/ids.json?cursor=-1&screen_name={user}&count={count}"
		)
		...
		return TwitterScraperFollowing(
			twitter_data=data
		)
